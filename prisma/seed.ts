...
async function main() {
  ...
  for (const item of items) {
    ...
    console.log(`Created item with name ${newItem.name}, amount: ${newItem.amount}, categories: ${item.categories.length}`);
  }
  ...
  const loanStatuses: Array<{ status: "ACCEPTED" | "REJECTED" | "INUSE" | "IN_BOX" | "RETURNED", probability: number }> = [
  ...
  for (let i = 0; i < loanCount; i++) {
    ...
    const loan = await prisma.loan.create({
      data: {
        ...
        boxId: selectedStatus === "IN_BOX" ? boxes[Math.floor(Math.random() * boxes.length)].id : null,
        reservations: {
    ...
    console.log(`Created loan ${i + 1}/${loanCount}: status=${loan.status}, items=${loan.reservations.length}, user=${user.name}`);
  }

  console.log(`Seeding finished successfully!`);
  console.log(`Summary:`);
  console.log(`- Users: ${createdUsers.length + 2} (${createdUsers.length} regular + 1 admin + 1 kiosk)`);
