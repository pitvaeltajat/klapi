...
export default function LoanEditView({ loan, items }) {
    ...
    const [startDate, setStartDate] = useState(loan.startTime.toISOString().split('.')[0]);
    const handleStartDateChange = (e) => setStartDate(e.target.value);
    const [endDate, setEndDate] = useState(loan.endTime.toISOString().split('.')[0]);
    ...
    return (
        <div>
            ...
            <Stack spacing={3} direction='row'>
                <Textarea
                    borderColor={description != loan.description ? 'orange.200' : 'gray.300'}
                ...
                />
                <IconButton aria-label='Reset' icon={<RepeatClockIcon />} onClick={() => setDescription(loan.description)} />
            </Stack>
            ...
            <Stack direction='row'>
                <Input
                    borderColor={startDate != loan.startTime.toISOString().split('.')[0] ? 'orange.200' : 'gray.300'}
                    borderWidth={startDate != loan.startTime.toISOString().split('.')[0] ? '2px' : '1px'}
                ...
                <IconButton
                    ...
                    onClick={() => setStartDate(loan.startTime.toISOString().split('.')[0])}
                />
            </Stack>
            ...
            <Stack direction='row'>
                <Input
                    borderColor={endDate != loan.endTime.toISOString().split('.')[0] ? 'orange.200' : 'gray.300'}
                    borderWidth={endDate != loan.endTime.toISOString().split('.')[0] ? '2px' : '1px'}
                        ...
                        {reservations.map((reservation) => {
                            return (
                                <Tr key={reservation.id}>
                                    <Td
                                        color={
                                            loan.reservations.filter((r) => r.id == reservation.id).length > 0
                                                ? 'gray.900'
                                                : 'orange.500'
                                        }
                                    >
                                        ...
                                        <InputGroup>
                                            <InputLeftAddon>
                                                <IconButton
                                                    ...
                                                    isDisabled={reservation.amount == 0}
                                                />
                                            </InputLeftAddon>
                                            <Input
                                                borderColor={
                                                    loan.reservations.filter((r) => r.id == reservation.id).length > 0
                                                        ? reservation.amount ==
                                                          loan.reservations.filter((r) => r.id == reservation.id)[0].amount
                                                            ? 'gray.300'
                                                            : 'orange.200'
                                                        : 'gray.300'
                                                }
