...
import { Stack, Box, Button, Heading, ButtonGroup, HStack, Wrap, WrapItem, Link } from '@chakra-ui/react';
	...
	return (
		<Link as={NextLink} href={`/loan/${loan.id}`}>
			<Box width='100%' key={loan.id}>
				...
				<Box padding={2} display='flex' flexDirection='row' justifyContent='space-between' backgroundColor='gray.100'>
					<Stack spacing='5px'>
	...
	return (
		<Stack spacing={5}>
			<Wrap spacing={2}>
				...
				<WrapItem>
					<Button colorScheme={'yellow'} onClick={() => setLoanCategory('PENDING')}>
						Odottaa käsittelyä
					</Button>
				</WrapItem>
				<WrapItem>
					<Button colorScheme={'green'} onClick={() => setLoanCategory('ACCEPTED')}>
						Hyväksytyt
					</Button>
				</WrapItem>
				<WrapItem>
					<Button colorScheme={'red'} onClick={() => setLoanCategory('REJECTED')}>
						Hylätyt
					</Button>
				</WrapItem>
				<WrapItem>
				...
				<WrapItem>
					<Button colorScheme={'purple'} onClick={() => setLoanCategory('RETURNED')}>
						Palauteut
					</Button>
				</WrapItem>
			</Wrap>
			...
			{LoanCategory === 'ALL'
				...
				: loans.filter((loan) => loan.status === LoanCategory).map((loan) => <LoanCard key={loan.id} loan={loan} />)}
		</Stack>
	);
}
