...
export interface ReturnableReservation {
  id: string;
  amount: number;
...
export default function LoanReturnDialog({
  loanId,
  reservations,
  onReturnComplete,
}: {
  loanId: string;
  reservations: ReturnableReservation[];
  ...
  const inuseReservations = React.useMemo(
    () => reservations.filter((r) => r.status === ReservationStatus.INUSE),
    [reservations],
  );
  ...
  const submitReturn = async (): Promise<{ name: string; description: string | null } | null> => {
    try {
      const response = await fetch('/api/loan/loanReturned', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: loanId, reservationIds: Array.from(selectedIds) }),
      });
